...
import net.mint.utils.graphics.impl.elements.GradientTextUtil;
...
public class HudFeature extends Feature {
    ...
    public BooleanSetting showRevision = new BooleanSetting("ShowRevision", "Show git revision in watermark.", true, () -> watermark.getValue() && !Mint.GIT_REVISION.isEmpty());
    public BooleanSetting showHash = new BooleanSetting("ShowHash", "Show git hash in watermark.", true, () -> watermark.getValue() && !Mint.GIT_HASH.isEmpty());
    ...
    public void renderHud(RenderHudEvent event) {
        ...
        if (wmAlpha > 0 && watermark.getValue()) {
            ...
            boolean hasVersionInfo = (showVersion.getValue() && !Mint.MOD_VERSION.isEmpty())
                    || (showRevision.getValue() && !Mint.GIT_REVISION.isEmpty())
                    || (showHash.getValue() && !Mint.GIT_HASH.isEmpty());

            if (hasName) {
                text.append(Text.literal(Mint.NAME));
            }
            ...
            if (hasVersionInfo) {
                ...
                if (showVersion.getValue()) {
                    versionPart.append("v").append(Mint.MOD_VERSION);
                }

                StringBuilder suffix = new StringBuilder();
                if (showRevision.getValue() && !Mint.GIT_REVISION.isEmpty()) {
                    suffix.append(Mint.GIT_REVISION);
                }
                if (showHash.getValue() && !Mint.GIT_HASH.isEmpty()) {
                    if (suffix.length() > 0) suffix.append(".");
                    suffix.append(Mint.GIT_HASH);
                }
