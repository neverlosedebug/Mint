...
public class NumberButton extends Button implements Globals {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        if (listening) {
            drawTextWithShadow(context, currentString + (Mint.CLICK_GUI.isLine() ? "|" : " "), getX() + (getWidth()/2) - (FontUtils.getWidth(currentString) / 2.0f), getY() + getVerticalPadding(), Color.WHITE);
        } else {
    ...
    public void keyPressed(int keyCode, int scanCode, int modifiers) {
        ...
        if (InputUtil.isKeyPressed(mc.getWindow().getHandle(), GLFW.GLFW_KEY_ENTER)) {
            try {
                ...
                Mint.getLogger().error("Invalid value, expected a {}.", setting.getType());
            }
            listening = false;
            return;
        }
