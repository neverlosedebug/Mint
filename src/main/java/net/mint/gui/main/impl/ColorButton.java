    ...
    public void keyPressed(int keyCode, int scanCode, int modifiers) {
        ...
        if (ctrl) {
            if (keyCode == GLFW.GLFW_KEY_C) {
                Mint.CLICK_GUI.setColorClipboard(setting.getColor());
                return;
            }

            if (keyCode == GLFW.GLFW_KEY_V && Mint.CLICK_GUI.getColorClipboard() != null) {
                setting.setColor(Mint.CLICK_GUI.getColorClipboard());
                hsb = getHSB();
            }
        }
    }
